...
export const CellAction: React.FC<CellActionProperties> = ({ data }) => {
  ...
  const onConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${parameters.storeId}/sizes/${data.id}`);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onClick={() => onCopy(data.id)}>
            <Copy className="mr-2 size-4" /> Копировать идентификатор размера
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => router.push(`/${parameters.storeId}/sizes/${data.id}`)}>
