...
export async function POST(req: Request, {params}: { params: { storeId: string; }; }) {
    try {
        ...
        if (!categoryId) {
            return new NextResponse("Необходим идентификатор категории.", {status: 400});
        }

        if (!colorId) {
            return new NextResponse("Необходим идентификатор цвета.", {status: 400});
        }
        ...
        if (!params.storeId) {
            return new NextResponse("Необходим идентификатор магазина.", {status: 400});
        }
        ...
        const product = await prismadb.product.create({
            data: {
                name, price, isFeatured, isArchived, categoryId, colorId, sizeId, storeId: params.storeId, images: {
                    createMany: {
    ...
    try {
        ...
        const products = await prismadb.product.findMany({
            where: {
                ...
                images: true, category: true, color: true, size: true,
            }, orderBy: {
                createdAt: 'desc',
            }
        });
