...
export async function GET(req: Request, {params}: { params: { productId: string; }; }) {
    try {
        if (!params.productId) {
            return new NextResponse("Необходим идентификатор товара.", {status: 400});
        }
        ...
        const product = await prismadb.product.findUnique({
            where: {
                ...
                images: true, category: true, size: true, color: true,
            }
        });
    ...
    try {
        ...
        await prismadb.product.update({
            where: {
                ...
                name, price, categoryId, colorId, sizeId, images: {
                    deleteMany: {},
